...
$columns = array(
    ...
    array( 
      ...
      'dt' => 'purchase',
      'formatter' => function( $d, $row ) {
        return number_format($row['item_purchase_price']);
      }
    ),
    array( 
      ...
      'dt' => 'sell',
      'formatter' => function( $d, $row ) {
        return number_format($row['item_selling_price']);
      }
    ),
    array( 
      'db' => 'return_quantity',
      'dt' => 'return_quantity',
      'formatter' => function( $d, $row ) {
        return number_format($row['return_quantity']);
      }
    ),
    array( 
      'db' => 'item_quantity',
      'dt' => 'quantity',
      'formatter' => function( $d, $row ) {
        return number_format($row['item_quantity']-$row['return_quantity']);
      }
    ),
    array( 
      'db' => 'total_sell',  
      'dt' => 'sold',
      'formatter' => function( $d, $row ) {
        return number_format($row['total_sell']);
      }
    ),
    array(
      'db' => 'invoice_id',  
      'dt' => 'available',
      'formatter' => function( $d, $row ) {
        return number_format($row['item_quantity'] - ($row['total_sell']+$row['return_quantity']));
      }
    ),
    array( 
      'db' => 'status',  
