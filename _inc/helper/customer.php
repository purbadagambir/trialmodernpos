...
function get_customer_previus_balance($customer_id)
{
	$customer_model = registry()->get('loader')->model('customer');
	return $customer_model->getPreviusBalance($customer_id);
}
...
{
	$customer_model = registry()->get('loader')->model('customer');
	return $customer_model->getCustomerReference($customer_id);
}
