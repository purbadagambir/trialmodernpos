...
function get_postemplate_data($invoice_id)
{
    ...
    $data = array(
        ...
        'total_points' => get_the_customer($invoice_info['customer_id'], 'total_points'),
        'point_trx' => $invoice_info['point_trx'],
        ...
        'interest_amount' => number_format($invoice_info['interest_amount']),
        'interest_percentage' => number_format($invoice_info['interest_percentage']),
        'item_tax' => number_format($invoice_info['item_tax']),
        'order_tax' => number_format($invoice_info['order_tax']),
        'total_tax' => number_format($invoice_info['item_tax'] + $invoice_info['order_tax']),
        'cgst' => number_format($invoice_info['cgst']),
        'sgst' => number_format($invoice_info['sgst']),
        'igst' => number_format($invoice_info['igst']),
        'total_purchase_price' => number_format($invoice_info['total_purchase_price']),
        'shipping_type' => '',
        'shipping_amount' => number_format($invoice_info['shipping_amount']),
        'others_charge' => number_format($invoice_info['others_charge']),
        'payable_amount' => number_format($invoice_info['payable_amount'] + $invoice_info['previous_due']),
        'paid_amount' => number_format($invoice_info['paid_amount'] + ($invoice_info['balance'] - $invoice_info['return_amount'])),
        'due' => number_format($invoice_info['due'] + $invoice_info['previous_due']),
        'due_paid' => number_format($invoice_info['due_paid']),
        'previous_due' => number_format($invoice_info['previous_due']),
        'prev_due_paid' => number_format($invoice_info['prev_due_paid']),
        'profit' => number_format($invoice_info['profit']),
        'return_amount' => number_format($invoice_info['return_amount']),
        'change' => number_format($invoice_info['balance']),
        'balance_to_credit' => number_format($payments[0]['balance_to_credit']),
        'balance_to_cash' => number_format($invoice_info['balance'] - $payments[0]['balance_to_credit']),
    ...
    foreach ($invoice_items as $item) {
        ...
        foreach ($item as $key => $val) {
            if (in_array($key, array('sl'))) {
                ...
                $new_item[$key] = number_format($val - ($item['item_tax']) * $item['item_quantity']);
            } else {
                $new_item[$key] = $val;
            }
        }
    ...
    foreach ($return_items as $item) {
        ...
        foreach ($item as $key => $val) {
            if (in_array($key, array('sl'))) {
                ...
                $new_item[$key] = number_format($val);
            }
        }
        $item_array[] = $new_item;
        $inc++;
    ...
    foreach ($payments as $payment) {
        ...
        foreach ($payment as $key => $val) {
            if (in_array($key, array('sl'))) {
                ...
                $new_payment[$key] = $payment['total_paid'] > 0 ? number_format($payment['total_paid']) : number_format($payment['amount']);
            } elseif ($key == 'name') {
                ...
                $new_payment[$key] = number_format($val);
            }
        }
...
function get_postemplate_empty_data()
{
    $data = array(
        ...
        'subtotal' => number_format(600),
        'discount_type' => 'Percentage',
        'discount_amount' => number_format(10),
        'interest_amount' => number_format(40),
        'interest_percentage' => number_format(10),
        'item_tax' => number_format(30),
        'order_tax' => number_format(100),
        'total_tax' => number_format(20),
        'cgst' => number_format(30),
        'sgst' => number_format(10),
        'igst' => number_format(18),
        'total_purchase_price' => number_format(500),
        'shipping_type' => '',
        'shipping_amount' => number_format(50),
        'others_charge' => number_format(50),
        'payable_amount' => number_format(800),
        'paid_amount' => number_format(500),
        'due' => number_format(50),
        'due_paid' => number_format(100),
        'previous_due' => number_format(100),
        'prev_due_paid' => number_format(100),
        'profit' => number_format(10),
        'return_amount' => number_format(10),
        'change' => number_format(50),
        'balance_to_credit' => number_format(50),
