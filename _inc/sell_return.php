...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'RETURN') {
  try {
    ...
    foreach ($items as $item) {
      ...
      // Begin : Update By Henry @ 30/06/2022
      $return_qty_conversi =  $item_quantity * $invoice_item['sell_vol_unit'];
      $item_quantity_exist = $return_qty_conversi;
      $return_quantity = $return_qty_conversi;
      // End : Update By Henry @ 30/06/2022
      ...
      $sell_unit_id = $invoice_item['sell_unit_id'];
      $sell_vol_unit = $invoice_item['sell_vol_unit'];

      $item_name = $invoice_item['item_name'];
      $item_price = $invoice_item['item_price'];
      $per_item_discount = $invoice_item['item_discount'] / $invoice_item['item_quantity'];
      ...
      if ($tax_method == 'exclusive') {
        ...
        $subtotal = ($item_price * $item_quantity) - ($per_item_discount * $item_quantity);
      ...
      //UPDATE BY AKBAR 25/06/22
      $statement = db()->prepare("INSERT INTO `return_items` SET `store_id` = ?, `reference_no` = ?, `item_id` = ?, `item_name` = ?, `item_quantity` = ?, `item_price` = ?, `item_purchase_price` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `item_total` = ?, `created_at` = ?, `return_unit_id` = ?, `return_vol_unit` = ?,`return_qty_conversi` = ?");
      $statement->execute(array($store_id, $reference_no, $item_id, $item_name, $item_quantity, $item_price, $item_purchase_price, $item_tax, $cgst, $sgst, $igst, $item_total, date_time(), $sell_unit_id, $sell_vol_unit, $return_qty_conversi));
    };
    ...
    $statement = db()->prepare("INSERT INTO `returns` SET `store_id` = ?, `reference_no` = ?, `invoice_id` = ?, `purchase_invoice_id` = ?, `customer_id` = ?, `note` = ?, `total_item` = ?, `total_quantity` = ?, `subtotal` = ?, `total_amount` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `total_purchase_price` = ?, `profit` = ?, `created_by` = ?, `created_at` = ?, `updated_at` = ?, total_points = ?");
    $statement->execute(array($store_id, $reference_no, $invoice_id, $purchase_invoice_id, $customer_id, $note, $total_item, $total_quantity, $tsubtotal, $tpayable, $titem_tax, $tcgst, $tsgst, $tigst, $total_purchase_price, $profit, $user_id, date_time(), date_time(), $point));

    /* Update By Henry @ 18-05-2022*/
    $statement = db()->prepare("CALL sp_calc_sharing_point(?,?)");
    $statement->execute(array($reference_no, 'return'));
