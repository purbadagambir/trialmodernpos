...
if (!isset($request->get['invoice_id'])) {
    header('HTTP/1.1 422 Unprocessable Entity');
    header('Content-Type: application/json; charset=UTF-8');
    echo json_encode(array('errorMsg' => trans('error_pos_printer')));
    exit();
}
...
if (store('remote_printing') != 1) {
    ...
    <script type="text/javascript">
        ...
        function receiptData() {
            ...
            receipt.info = "";
            receipt.info .= "<?php echo trans("label_datetime") . ": " . format_date($invoice_info['created_at']); ?>" + "\n";
            receipt.info .= "<?php echo trans("label_invoice_id") . ": " . $invoice_id; ?>" + "\n";
            receipt.info .= "<?php echo trans("label_created_by") . ": " .get_the_user($invoice_info['created_by'], 'username'); ?>" + "\n\n";
            receipt.info .= "<?php echo trans("label_customer") . ": " . ($customer_name); ?>" + "\n";
            receipt.info .= "<?php echo trans("label_mobile_no") . ": " . ($customer_contact); ?>" + "\n";
            receipt.info .= "\n";
            ...
            receipt.items .= "<?php echo printLine("   ".($row['item_quantity'])." x ".number_format($row['item_price']) . ":  ". number_format($row['item_quantity']*$row['item_price']), $char_per_line, ' '); ?>" + "\n";
            <?php $r++; endforeach; ?>

            receipt.totals = "";
            receipt.totals .= "<?php echo printLine(trans('label_subtotal') . ": " . number_format($invoice_info['payable_amount']), $char_per_line); ?>" + "\n";
            <?php
            receipt.totals .= "<?php echo printLine(trans('label_discount') . ": " . number_format($invoice_info['discount_amount']), $char_per_line); ?>" + "\n";
            <?php
            receipt.totals .= "<?php echo printLine(trans('label_tax') . ": " . number_format($invoice_info['tax_amount']), $char_per_line); ?>" + "\n";
            <?php
            ?>
            receipt.totals .= "<?php echo printLine(trans("label_grand_total") . ": " . number_format($invoice_info['payable_amount']), $char_per_line); ?>" + "\n";
            receipt.totals .= "<?php echo printLine(trans("label_paid_amount") . ": " . number_format($invoice_info['paid_amount']), $char_per_line); ?>" + "\n";
            receipt.totals .= "<?php echo printLine(trans("label_due_amount") . ": " . number_format($invoice_info['due']), $char_per_line); ?>" + "\n";
