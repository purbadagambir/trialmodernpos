...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'WITHDRAW')
{
  try {
    ...
    echo json_encode(array('msg' => trans('text_withdraw_success'), 'invoice' => $invoice, 'balance' => number_format(get_bank_balance(from(), to()))));
    exit();
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DEPOSIT')
{
  try {
    ...
    header('Content-Type: application/json');
    echo json_encode(array('msg' => trans('text_deposit_success'), 'invoice' => $invoice, 'balance' => number_format(get_bank_balance(from(), to()))));
    exit();
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'TRANSFER')
{
  try {
    ...
    header('Content-Type: application/json');
    echo json_encode(array('msg' => trans('text_transfer_success'), 'invoice' => $invoice, 'balance' => number_format(get_bank_balance(from(), to()))));
    exit();
...
$columns = array(
    ...
    array( 
      ...
      'formatter' => function($d, $row) {
        ...
        $withdraw = $row['amount'];
        return number_format($withdraw, 2);
      }
    ),
    array( 
      'db' => 'ref_no',   
