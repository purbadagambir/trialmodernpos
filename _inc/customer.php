...
function validate_request_data($request) 
{
  ...
  if (!validateInteger($request->post['parent_id'])) {
    throw new Exception(trans('error_parent_id'));
  }    
...
// DB table to use
$table = "(SELECT B.customer_name as parent_name,customers.*,concat('62',right(customers.customer_mobile,length(customers.customer_mobile) -1)) as wa_no , c2s.balance, c2s.status, c2s.sort_order FROM customers LEFT JOIN customer_to_store c2s ON (customers.customer_id = c2s.customer_id) left join (select parent_id,customer_name,customer_id from customers) B on customers.parent_id=B.customer_id
...
$columns = array(
  ...
  array(
      ...
      'formatter' => function( $d, $row ) {
        ...
        if (!$row['status']) {
          return '<a href="#" class="btn btn-sm btn-block btn-default" type="button" disabled><i class="fa fa-comments-o"></i></a>';
        }
        
        return '<a target="_blank" href="https://api.whatsapp.com/send?phone='.$row['wa_no'].'" data-action="share/whatsapp/share" id="sell-product" class="btn btn-block btn-success" type="button" title="'.trans('button_WhatsApp').'"><i class="fa fa-comments-o"></i></a>';
      }
  ),
  array(
      'db'        => 'customer_sex',
  ...
  array( 'db' => 'parent_name', 'dt' => 'parent_name' ),
  array(
      'db'        => 'customer_id',
      'dt'        => 'btn_pos',
      'formatter' => function( $d, $row ) {
        ...
        return '<a href="pos.php?customer_id='.$row['customer_id'].'" id="sell-product" class="btn btn-sm btn-block btn-warning" type="button" title="'.trans('button_sell').'"><i class="fa fa-shopping-cart"></i></a>';
      }
  ),
  array(
      'db'        => 'customer_id',
