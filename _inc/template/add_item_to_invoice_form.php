					...
					<tbody>
						<tr ng-repeat="items in order.items">
							<td class="text-center w-10">
								...
								<input type="hidden" name="item['{{ items.item_id ? items.item_id : items.group_id }}'][item_price]" value="{{ items.item_price  | number }}">
								<input type="hidden" name="item['{{ items.item_id ? items.item_id : items.group_id }}'][item_quantity]" value="{{ items.item_quantity }}">
								<input type="hidden" name="item['{{ items.item_id ? items.item_id : items.group_id }}'][item_total]" value="{{ items.item_total  | number }}">
								{{ $index+1 }}
							</td>
							<td class="w-70">{{ items.item_name }} (x{{ items.item_quantity | number }})</td>
							<td class="text-right w-20">{{ items.item_total  | number }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							...
							<td class="text-right w-40">{{ order.subtotal  | number }}</td>
						</tr>
						<tr>
							<th class="text-right w-60"  colspan="2">
							...
							<td class="text-right w-40" >{{ order.discount_type  == 'percentage' ? (_percentage(order.payable_amount, order.discount_amount) | number) : (order.discount_amount | number) }}</td>
						</tr>
						<tr>
							<th class="text-right w-60" colspan="2">
							...
							<td class="text-right w-40">{{ order.tax_amount  | number }}</td>
						</tr>
						<tr>
							<th class="text-right w-60" colspan="2">
							...
							<td class="text-right w-40">{{ order.previous_due  | number }}</td>
						</tr>
						<tr>
							<th class="text-right w-60" colspan="2">
							...
							<td class="text-right w-40">{{ order.payable_amount | number }}</td>
						</tr>
						...
						<tr class="success" ng-repeat="payments in order.payments">
							...
							<td class="text-right w-40">{{ payments.amount | number }}</td>
						</tr>
						<!-- Payments end -->
						...
						<tr class="danger">
							...
							<td class="text-right w-40">{{ order.due | number }}</td>
						</tr>

						<tr class="warning">
							<th class="text-right w-60" colspan="2">Balance</th>
							<td class="text-right w-40">{{ order.balance | number }}</td>
						</tr>
