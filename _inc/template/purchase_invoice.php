				...
				<table class="table table-bordered margin-b0 table-condensed">
					...
					<tbody>
						...
						foreach ($invoice_items as $product) : ?>
							<tr>
								<td>
									<?php echo $product['item_name']; ?> ( x<?php echo number_format($product['item_quantity']); ?> <?php echo $product['unitName']; ?> )
								</td>
								<td class="text-right">
									<?php echo number_format($product['item_purchase_price']); ?>
								</td>
								<td class="text-right">
									<?php 
									$subtotal += $product['item_total'];
									echo number_format($product['item_total']); ?>
								</td>
							</tr>
						<?php endforeach; ?>
					</tbody>
					<tfoot>
						...
						<tr class="bg-gray">
							...
							<td class="w-20 text-right">
								<?php echo number_format($invoice['order_tax']); ?>
							</td>
						</tr>
						<tr class="bg-gray">
							<td class="text-right" colspan="2">
							...
							<td class="w-20 text-right">
								<?php echo number_format($invoice['shipping_amount']); ?>
							</td>
						</tr>
						<tr class="bg-gray">
							<td class="text-right" colspan="2">
							...
							<td class="w-20 text-right">
								<?php echo number_format($invoice['others_charge']); ?>
							</td>
						</tr>
						<tr class="bg-gray">
							<td class="text-right" colspan="2">
						...
						<tr class="bg-gray">
							...
							<td class="w-20 text-right">
								<?php echo number_format($invoice['paid_amount']); ?>
							</td>
						</tr>
						<tr class="danger">
							<td class="text-right" colspan="2">
							...
							<td class="w-20 text-right">
								<?php echo number_format($invoice['due']); ?>
							</td>
						</tr>
					</tfoot>
				</table>
					...
					<tbody>
                        <?php $inc=1;foreach ($returns as $row):?>
                          <tr>
                            ...
                            <td class="w-25 text-center"><?php echo number_format($row['item_quantity']);?> <?php echo get_the_unit(get_the_product($row['item_id'],'unit_id'), 'unit_name');?></td>
                          <t/r>
                        <?php $inc++;endforeach; ?>
					</tbody>
				</table>
                        ...
                        <?php foreach ($payments as $row) : 
                          ...
                          <tr class="bt-1 <?php echo $color;?>">
                            ...
                            <td class="w-25 text-right">
                              <?php if ($row['type'] == 'return') : ?>
                                <?php echo trans('label_amount'); ?>:&nbsp; <?php echo number_format($row['amount']); ?>
                              <?php elseif ($row['type'] == 'change') : ?>
                                &nbsp;
                              <?php else : ?>
                                <?php echo trans('label_amount'); ?>:&nbsp; <?php echo number_format($row['total_paid']); ?>
                              <?php endif; ?>
                            </td>
                            <td class="w-25 text-right">
                              <?php if ($row['type'] != 'return' && $row['balance'] > 0) : ?>
                                <?php echo trans('label_change'); ?>:&nbsp; <?php echo number_format($row['balance']); ?>
                              <?php else: ?>
                                &nbsp;
                              <?php endif; ?>
                            </td>
