			...
			<div class="col-lg-5 col-md-5 col-sm-5 checkout-payment-option bootboox-container">
				<div class="tab-wrapper tab-cheque bootboox-container tab-cheque-payment">
					...
					<div class="input-group input-group-lg pmethod-field-wrapper">
						...
						<input id="paid-amount" class="form-control" type="number" name="paid-amount" ng-model="paidAmount" placeholder="<?php echo trans('placeholder_input_an_amount'); ?>" ng-keypress="checkoutWhilePressEnter($event)" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" onKeyUp="if(this.value<0){this.value='1';}">
					</div>

					<!-- /* Start update by henry @09-06-2022 */ -->
					<div class="input-group input-group-lg pmethod-field-wrapper mt-5">
						<span class="input-group-addon"><?php echo trans('text_Bal_To_Credit'); ?></span>
						<input id="paid-amount" class="form-control" type="text" name="balance-to-credit" ng-model="balanceToCredit" placeholder="<?php echo trans('placeholder_input_an_balance_to_credit'); ?>" ng-keypress="checkoutWhilePressEnter($event)" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" onKeyUp="if(this.value<0){this.value='1';}">
					</div>
					<!-- /* End update by henry @09-06-2022 */ -->
						...
						<div ng-show="isInstallmentOrder" class="panel panel-default mt-5">
							<div class="panel-body">
								...
								<div class="form-group">
									...
									<div class="col-sm-6">
										<input type="text" id="installment_count" name="installment_count" value="{{ installmentDuration/installmentIntervalCount | number }}" class="form-control text-right" readonly>
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-6">
										<input type="text" id="installment_interest_amount" name="installment_interest_amount" value="{{ installmentInterestAmount | number}}" class="form-control text-right" readonly>
									</div>
								</div>
							</div>
						</div>
					...
					<table class="table table-bordered table-striped table-condensed">
						...
						<tfoot>
							...
							<tr>
								...
								<td class="text-right w-40">{{ taxAmount  | number }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
								...
								<td class="text-right w-40">{{ shippingType  == 'percentage' ? (_percentage(totalAmount, shippingAmount) | number) : (shippingAmount | number) }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
								...
								<td class="text-right w-40">{{ othersCharge | number }}</td>
							</tr>
							<?php if (INSTALLMENT) : ?>
								<tr>
									<th class="text-right w-60" colspan="2">
									...
									<td class="text-right w-40">{{ installmentInterestAmount  | number }}</td>
								</tr>
							<?php endif; ?>
							<tr>
								<th class="text-right w-60" colspan="2">
								...
								<td class="text-right w-40">{{ dueAmount  | number }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
								...
								<td class="text-right w-40">{{ totalPayable  | number }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
									<?php echo trans('label_paid_amount'); ?>
								</th>
								<td class="text-right w-40">{{ paidAmount | number }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
									<?php echo trans('label_due_amount'); ?>
								</th>
								<td class="text-right w-40">{{ totalPayable < paidAmount ? totalPayable : totalPayable - paidAmount | number }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
									<?php echo trans('label_balance'); ?>
								</th>
								<td class="text-right w-40">{{ totalPayable < paidAmount ? paidAmount - totalPayable  : 0 | number }}</td>
							</tr>
							<!-- Start update by henry @09-06-2022 -->
							<tr>
								<th class="text-right w-60" colspan="2">
									<?php echo trans('label_balance_to_credit'); ?>
								</th>
								<td class="text-right w-40">{{ balanceToCredit | number }}</td>
							</tr>
							<tr>
								<th class="text-right w-60" colspan="2">
									<?php echo trans('label_balance_to_cash'); ?>
								</th>
								<td class="text-right w-40">{{ totalPayable < paidAmount ? paidAmount - totalPayable - balanceToCredit  : 0 | number }}</td>
							</tr>
							<!-- End update by henry @09-06-2022 -->
