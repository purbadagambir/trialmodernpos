...
if ($request->server['REQUEST_METHOD'] == 'POST' && $action_type == 'SEND') 
{
	try {
		...
		if (!$message) {
			...
			// Dear [customer_name],  Invoice ID: [invoice_id],  Payable: [payable_amount],  Paid: [paid_amount] ,  Due: [due]. Purchase at- [date_time]. Regards, [store_name],  [address]
		}
		...
		$data = array(
			...
			'discount' => number_format($invoice['discount_amount']),
			'payable_amount' => get_currency_code().' '.number_format($invoice['payable_amount']),
			'paid_amount' => get_currency_code().' '.number_format($invoice['paid_amount']),
			'due' => get_currency_code().' '.number_format($invoice['due']),
			...
			'tax' => number_format($invoice['item_tax']+$invoice['order_tax']),
		);
