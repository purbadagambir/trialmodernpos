...
class ModelReport extends Model
{
	...
	public function getTotalCashReceivedBy($type, $type_id, $from = null, $to = null, $store_id = null)
	{
		...
		switch ($type) {
			case 'userwise':
				...
				// $due_collection = $this->getTotalDueCollectionBy($type_id, $from, $to);
				// $total = $total + $due_collection;
				break;
			case 'invoicewise':
	...
	public function getTotalCashCollectionBy($user_id, $from = null, $to = null)
	{
		$total = 0;
		$from = $from ? $from : date('Y-m-d');
		$to = $to ? $to : date('Y-m-d');
		$where_query = "`payments`.`type`='sell' AND `payments`.`created_by` = ?";
		$where_query .= date_range_sell_payments_filter($from, $to);
		$statement = $this->db->prepare("SELECT SUM(`payments`.`amount`) as total, `created_at` FROM `payments` 
			WHERE $where_query");
		$statement->execute(array($user_id));
	...
	public function getTotalCreditToBalanceBy($user_id, $from = null, $to = null)
	{
		$total = 0;
		$from = $from ? $from : date('Y-m-d');
		$to = $to ? $to : date('Y-m-d');
		$where_query = "`payments`.`type`='sell' AND `payments`.`created_by` = ?";
		$where_query .= date_range_sell_payments_filter($from, $to);
		$statement = $this->db->prepare("SELECT SUM(`payments`.`balance_to_credit`) as total, `created_at` FROM `payments` 
			WHERE $where_query");
		$statement->execute(array($user_id));
		$invoice = $statement->fetch(PDO::FETCH_ASSOC);
		return $invoice['total'];
