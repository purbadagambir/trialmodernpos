...
class ModelInvoice extends Model
{
    ...
    public function createInvoice($request, $store_id = null)
    {
        ...
        $balance_to_credit = $request->post['balance-to-credit'] ? $request->post['balance-to-credit'] : 0;
        ...
        if ($balance_to_credit > $balance) {
            throw new Exception(trans('error_balance_to_credit_amount_greater_than_due'));
        }
        ...
        foreach ($product_items as $product) {
            ...
            $product_volume = $product['item_vol_unit'] ? $product['item_vol_unit'] : 1;
            $product_quantity = $product['item_quantity'];
            $total_qty_conversi = $product_quantity * $product_volume;
            ...
            $item_tax += $tax;
            $quantity_exist = $total_qty_conversi;
            $sell_quantity = $total_qty_conversi;
            $inc = 1;
        ...
        /* Total purchase Item sudah di looping diatas tetapi ada BUGS jikalau terjadi stock yang harga belinya berbeda
           NOTE : Perhitungan total purchase yg salah hanya ada di table sell_price tetapi di sell_item benar
                  maka dari itu diambil solusi tercepat dengan melakukan sum ulang.
        */
        ...
        $statement->execute(array($total_points, $invoice_id, $store_id));

        /* Update by Henry @13-05-2022 */
        $statement = $this->db->prepare("CALL `sp_calc_sharing_point`(?,?) ");
        $statement->execute(array($invoice_id, 'sell'));

        if ($paid_amount > 0) {
            $statement = $this->db->prepare("INSERT INTO `payments` (store_id, invoice_id, pmethod_id, capital, amount, details, note, total_paid, pos_balance, created_by, created_at,balance_to_credit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($store_id, $invoice_id, $pmethod_id, $capital, $paid_amount, $details, $invoice_note, $total_paid, $balance, $user_id, $created_at, $balance_to_credit));
            ...
            if ($balance_to_credit > 0) {
                ...
                $statement = $this->db->prepare("UPDATE `customers` SET `credit` = $new_customer_balance WHERE  `customer_id` = ?");
                $statement->execute(array($customer_id));

                $statement = $this->db->prepare("UPDATE `customer_to_store` SET `balance` = $new_customer_balance WHERE  `customer_id` = ?");
                $statement->execute(array($customer_id));
            }
        }
    ...
    {
        $store_id = $store_id ? $store_id : store_id();
        /* Update by Akbar @ 13/05/2022 */
        $statement = $this->db->prepare("SELECT `selling_info`.*, `selling_price`.*, `customers`.`customer_id`, `customers`.`customer_name`, `customers`.`customer_mobile` AS `mobile_number`, `customers`.`customer_email` , `point` as `point_trx` FROM `selling_info` LEFT JOIN `selling_price` ON `selling_info`.`invoice_id` = `selling_price`.`invoice_id` LEFT JOIN `customers` ON `selling_info`.`customer_id` = `customers`.`customer_id` LEFT JOIN `point_logs` ON `selling_info`.`customer_id` = `point_logs`.`customer_id` and `selling_info`.`invoice_id` = `point_logs`.`trans_no`  WHERE `selling_info`.`store_id` = ? AND (`selling_info`.`invoice_id` = ? OR (`selling_info`.`customer_id` = ?) AND `selling_info`.`inv_type` = 'sell') ORDER BY `selling_info`.`invoice_id` DESC");
    ...
    public function getInvoiceItemsHTML($invoice_id, $store_id = null)
    {
        ...
        $html .= '<td class="text-right" style="padding:0 2px;">' . number_format($sell) . '</td>';
        $html .= '<td class="text-center" style="padding:0 2px;">' . number_format($qty) . '</td>';
        $html .= '<td class="text-right" style="padding:0 2px;">' . number_format($total) . '</td>';
        $html .= '</tr>';
        $html .= '</table>';
        return $html;
    }
