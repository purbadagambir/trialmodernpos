...
class ModelCustomer extends Model
{
	...
	{
		/* Update by Akbar @ 25-05-2022*/
		if (!empty($data['customer_email'])) {
			$email_pengirim = 'noreplay@pondo.co.id';
			$nama_pengirim = 'Pondo Network';
			$email_penerima = $data['customer_email'];
			$subjek = 'Your Account in Pondo Network';
			$id_user = $data['customer_email'];
			$pass_user = substr($data['customer_mobile'], -6);
		...
		$statement = $this->db->prepare("INSERT INTO `customers` (customer_name, dob, customer_email, customer_mobile,password, customer_sex, customer_age, gtin, customer_address, customer_city, customer_state, customer_country, created_at, parent_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$statement->execute(array($data['customer_name'], date('Y-m-d', strtotime($data['dob'])), $data['customer_email'], $data['customer_mobile'], password_hash(substr($data['customer_mobile'], -6), PASSWORD_DEFAULT), $data['customer_sex'], $data['customer_age'], $gtin, $data['customer_address'], $data['customer_city'], $customer_state, $data['customer_country'], date_time(), $data['parent_id']));
		$customer_id = $this->db->lastInsertId();

		$statement = $this->db->prepare("UPDATE `customers` set `status_network` = '1' WHERE customer_id = ? ");
		$statement->execute(array($data['parent_id']));

		/* Update by Henry @ 19-05-2022*/
		/* Diremark dikarenakan sudah diganti dengan tr_ins_c2s*/
		/* $statement= $this->db->prepare("CALL sp_ins_c2s(?)");
	...
	/* Update by Henry Purba @ 12/05/2022 */
	public function getCustomerReference($customer_id)
	{
		$statement = $this->db->prepare('SELECT CONCAT(customer_name," (",customer_mobile,")") customer_name from customers where customer_id = ? LIMIT 1');
		$statement->execute(array($customer_id));

		$reference_by = $statement->fetch(PDO::FETCH_ASSOC);
		return isset($reference_by['customer_name']) ? $reference_by['customer_name'] : 'None';
	}
	...
	public function getRecentCustomers($limit, $store_id = null)
	{
		...
		$statement = $this->db->prepare(
			"SELECT customers.* FROM `selling_info` 
			LEFT JOIN `customers` ON (`selling_info`.`customer_id` = `customers`.`customer_id`) 
