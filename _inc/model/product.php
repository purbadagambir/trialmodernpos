...
class ModelProduct extends Model
{
	...
	public function editProduct($product_id, $data)
	{
		...
		$hsn_code = isset($data['hsn_code']) ? $data['hsn_code'] : NULL;
		$statement = $this->db->prepare("UPDATE `products` SET `p_type` = ?, `p_name` = ?, `p_code` = ?, `hsn_code` = ?, `barcode_symbology` = ?, `category_id` = ?, `unit_id` = ?, `p_image` = ?, `description` = ?, `unit_id_medium` = ?, `unit_id_large` = ?, `vol_unit_medium` = ?, `vol_unit_large` = ?  WHERE `p_id` = ?");
		$statement->execute(array($data['p_type'], $data['p_name'], $data['p_code'], $hsn_code, $data['barcode_symbology'], $data['category_id'], $data['unit_id'], $data['p_image'], $data['description'], $data['unit_id_medium'], $data['unit_id_large'], $data['vol_unit_medium'], $data['vol_unit_large'], $product_id));
		$preference = isset($data['preference']) && !empty($data['preference']) ? serialize($data['preference']) : serialize(array());
