...
$columns = array(
  ...
  array(
    ...
    'dt' => 'cash_collection',
    'formatter' => function ($d, $row) use ($report_model, $from, $to) {
      $total = $report_model->getTotalCashCollectionBy($row['created_by'], $from, $to);
      if ((int)$total <= 0) {
        return '0.00';
      }
      return currency_format($total, 2);
  ...
  array(
    ...
    'dt' => 'due_collection',
    'formatter' => function ($d, $row) use ($report_model, $from, $to) {
      $total = $report_model->getTotalDueCollectionBy($row['created_by'], $from, $to);
      if ((int)$total <= 0) {
        return '0.00';
      }
      return currency_format($total, 2);
  ...
  array(
    'db' => 'created_by',
    'dt' => 'balance_to_credit',
    'formatter' => function ($d, $row) use ($report_model, $from, $to) {
      $received_amount = $report_model->getTotalCreditToBalanceBy($row['created_by'], $from, $to);
  ...
  array(
    ...
    'formatter' => function ($d, $row) use ($report_model, $from, $to) {
      ...
      $credit_to_balance = $report_model->getTotalCreditToBalanceBy($row['created_by'], $from, $to);
      $total_received = $received_amount + $credit_to_balance;
      return currency_format($total_received);
    }
  ),
);
