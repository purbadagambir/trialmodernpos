					...
					<div id="left-panel" class="pos-content" style="<?php echo $user->getPreference('pos_side_panel') == 'left' ? 'float:right' : null; ?>">
						<div class="contents">
							...
							<div id="total-amount">
								...
								<div id="created_at">
									<input class="data-field" ng-init="createdAt='<?php echo date('Y-m-d'); ?>'" type="text" name="created_at" value="<?php echo date('Y-m-d'); ?>" ng-model="createdAt" disabled>
								</div>
										...
										<table class="table table-hovered">
											<tbody>
												<tr ng-repeat="items in itemArray" class="invoice-item">
													<td class="product-quantity" id="invoice-item-{{ items.id }}" style="width:10%">
													...
													<td style="width:20%;">
														<center>
															<div class="dropdown{{ items.id }}">
																<button ng-click="dropdown(items.id)" class="dropbtn" style="width:80px;max-width:80px;border: 1px solid #ddd;padding-top:0;padding-bottom:0;" id="unitItem{{ items.id }}">{{items.unitName}} <i class="fa fa-angle-down"></i></button>
																<div id="myDropdown{{ items.id }}" class="dropdown-content">
																	<a href="#" ng-click="unitChange(items.id, items.unitName, items.sell_price, items.vol_small, items.unit_id_small)">{{ items.unitName }}</a>
																	<a href="#" ng-click="unitChange(items.id, items.unitName_medium, items.sell_price_medium, items.vol_medium, items.unit_id_medium)">{{ items.unitName_medium }}</a>
																	<a href="#" ng-click="unitChange(items.id, items.unitName_large, items.sell_price_large, items.vol_large, items.unit_id_large)">{{ items.unitName_large }}</a>
																</div>
															</div>
														</center>
													</td>
													...
													<td class="product-price" style="width : 10%;">
														<center>
															...
															<?php else : ?>
																<span id="price_{{items.id}}">{{ items.price }}</span>
															<?php endif; ?>
														</center>
													</td>
													...
													<td class="product-subtotal" style="width:15%;">
														<center>
															<span id="subTotal_{{items.id}}">{{ items.subTotal | number  }}</span>
														</center>
													</td>
													<!-- <td class="product-delete text-green pointer" ng-click="refreshItemFromInvoice($index, items.id)">
